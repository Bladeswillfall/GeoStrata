...
public final class OreVerticalDistributionCommands {
    ...
    private static int run(ServerCommandSource source) {
        ...
        bands.forEach((startY, count) -> {
            ...
            band.addProperty("share", finalTotal == 0 ? 0.0 : count / (double) finalTotal);
            bandJson.add(startY + ".." + (startY + BAND_HEIGHT - 1), band);
        });
        result.add("yBands", bandJson);

        GeoStrata.LOGGER.info("GEOSTRATA_IRON_VERTICAL_BENCHMARK {}", result);
        long finalCount = total;
        source.sendFeedback(
                () -> Text.literal("Iron vertical benchmark complete: " + finalCount + " blocks; see server log for 16-block Y bands."),
                false
        );
        return 1;
    }
