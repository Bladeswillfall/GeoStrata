...
require(lens_source.count("CorrelatedExperimentChunkOwnership.ownershipForChunk") >= 2,
        "baseline strata lenses must be prepared to check origin and destination chunk ownership")
require("mutable.getX()" in lens_source and "mutable.getZ()" in lens_source,
        "cross-chunk lens placement must be prepared to clip candidate blocks entering owned chunks")
...
print(
    "Validated correlated core staging: feature type + chunk-normalized suppression are prepared, "
